...
import ExpiryNotificationPopup from "@/components/ExpiryNotificationPopup";
...
export default async function DashboardPage() {
  ...
  const remaining = Math.ceil(
    (new Date(userRecord.endDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24)
  );
  ...
  const whatsappText = encodeURIComponent(
    `Hi, I am ${userRecord.name}. I would like to renew my ${userRecord.plan} subscription at Kiran's Fitness Club.`
  );
  const whatsappUrl = `https://wa.me/${phone}?text=${whatsappText}`;

  return (
    <main className="pt-32 pb-20 bg-background min-h-screen">
      {/* Subscription expiry popup — shown client-side if within 7 days or expired */}
      <ExpiryNotificationPopup
        daysRemaining={remaining}
        memberName={userRecord.name.split(" ")[0]}
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-0">
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-8">
          <div
            className={`card p-8 border-t-4 transition-colors ${
              isExpired
                ? "border-t-red-500 border border-red-500/20 bg-red-500/5"
                : isWarning
                ? "border-t-yellow-500 border border-yellow-500/20 bg-yellow-500/5"
          ...
          >
            ...
            {isExpired ? (
              <div>
                ...
                <p className="text-text-secondary">
                  ...
                  {new Date(userRecord.endDate).toLocaleDateString("en-IN")}. To continue accessing the gym, please
                  renew below.
                </p>
              </div>
            ) : (
              <div>
                <p className={`text-5xl font-heading font-bold mb-2 ${isWarning ? "text-yellow-500" : "text-white"}`}>
                  {remaining}{" "}
                  <span className="text-xl text-text-secondary font-normal uppercase tracking-widest">Days Left</span>
                </p>
                <p className="text-text-secondary">
                  Keep up the great work! You&apos;re currently on the{" "}
                  <strong className="text-white">{userRecord.plan}</strong> plan.
                </p>
              </div>
            )}
            ...
            <div className="mt-8 pt-6 border-t border-border/50">
              <a
                ...
                className={`w-full text-center block px-6 py-4 rounded-button font-bold text-black transition-transform hover:-translate-y-1 ${
                  isExpired || isWarning ? "bg-accent" : "bg-white/10 text-white hover:bg-white/20"
                }`}
              >
