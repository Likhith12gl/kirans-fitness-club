...
/** Safe server-side HTML sanitizer — strips only dangerous patterns.
 *  Content is admin-authored (trusted), so we only need to remove
 *  <script>, <iframe>, javascript: hrefs, and on* event attrs. */
...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  try {
    ...
    if (!post) return { title: "Post Not Found | Kiran's Fitness Club" };
    ...
    return {
      ...
      keywords: ["fitness blog", "Kiran's Fitness Club", "gym Anjananagar", "fitness tips Bangalore"],
      ...
      openGraph: {
        ...
        url: `https://kiransfitnessclub.in/blog/${params.slug}`,
      ...
      alternates: {
        canonical: `https://kiransfitnessclub.in/blog/${params.slug}`,
      },
    };
  } catch {
    return { title: "Blog | Kiran's Fitness Club" };
  }
}
  ...
  try {
    ...
    const articleSchema = {
      ...
      "author": {
        ...
        "url": "https://kiransfitnessclub.in"
      },
      "publisher": {
        "@type": "Organization",
        "name": "Kiran's Fitness Club",
        "logo": {
          "@type": "ImageObject",
          "url": "https://kiransfitnessclub.in/icon.png"
        }
      },
      "mainEntityOfPage": {
        "@type": "WebPage",
        "@id": `https://kiransfitnessclub.in/blog/${params.slug}`
      }
    };
    ...
    return (
      <main className="pt-32 pb-20 bg-background min-h-screen flex items-center justify-center">
        <div className="card p-8 border border-red-500/50 text-center max-w-lg">
          ...
          <p className="text-text-secondary mb-6">We encountered a technical issue while loading this blog post. Please try again shortly.</p>
